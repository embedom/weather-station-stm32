...
void DS18B20Sensor::handleTransactionComplete(void)
{
	...
	switch(_State)
	{
		case STATE_RESET_FOR_CONVERT:
		{
			...
			Status = startDmaTransmitReceive(SENSOR_CMD_ENCODE_FRAMES[CMD_SKIP_ROM_INDEX], _RxBuffer, FRAME_SIZE);
			break;
		}
		case STATE_SKIP_ROM_FOR_CONVERT:
		{
			_State = STATE_CONVERT_T;
			Status = startDmaTransmitReceive(SENSOR_CMD_ENCODE_FRAMES[CMD_CONVERT_TEMP_INDEX], _RxBuffer, FRAME_SIZE);
			break;
		}
		...
		case STATE_MATCH_ROM_FOR_READ:
		{
			...
			Status = startDmaTransmitReceive(SENSOR_CMD_ENCODE_FRAMES[CMD_READ_SCRATCHPAD_INDEX], _RxBuffer, FRAME_SIZE);
			break;
		}
...
Status DS18B20Sensor::startDmaTransmitReceive(const uint8_t *TxBuffer, uint8_t *RxBuffer, uint16_t Length)
{
...
void DS18B20Sensor::encodeMatchRomFrames(void)
{
	for(uint8_t Sensor = 0U; Sensor < _NumSensorsFound; Sensor++)
	{
		...
		{
			encodeByte(_RomCodes[Sensor][i], &_MatchRomTxBuf[Sensor][FRAME_SIZE + (i * FRAME_SIZE)]);
		}
	}
}
	...
	switch(_SearchState)
	{
		...
		case SRCH_WRITE_DIR:
		{
			...
			else
			{
				...
				if(calculateCrc(_SearchRomCode, ROM_CODE_SIZE - 1U) == _SearchRomCode[ROM_CODE_SIZE - 1U])
				{
					for(uint8_t i = 0U; i < ROM_CODE_SIZE; i++)
					{
